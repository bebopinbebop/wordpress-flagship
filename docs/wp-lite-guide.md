...
⚠️ Throughout each of these modules, the names of internal components and features are dynamically loaded from script input variables at the beginning of runtime. This helps enforce tagging throughout the project. More detail can be found [here](/docs/tagging-architecture.md).
...
- There is no load balancer, so traffic goes directly to the EC2 public endpoint. This could become overwhelmed under heavier traffic.
- There is no ACM certificate or HTTPS automation yet. For now, you would have to set that up manually.
...
🟢 NOTE: The CLI has default parameters set in `[default-value]`, which automatically apply by pressing `Enter`.

## 1. Starting the script
From the project root, type the following:
```bash
chmod 700 scripts/start-demo.sh # use this if you have not granted permissions yet
./scripts/start-demo.sh
```
The script checks your shell environment to ensure that the necessary packages are installed. It shows this by displaying green `[ok]` text for every required package.

## 2. Entering inputs for the script
For ease of use, much of the script is geared toward building a `wp-lite` deployment, so default values have been preloaded as suggestions that can be accepted by pressing `Enter`.

Below is a simplified line-by-line input example for configuring the script to build a `wp-lite` project, with comments explaining the goal of each value:

```bash
Environment: wp-lite, wp-rds, or wp-mig [wp-lite]: # which deployment you want: wp-lite, wp-rds, or wp-mig

Website display name [Cloud WordPress Demo]: # the name of the website, can be anything

Use a custom static demo folder instead of website/default-site? yes or no [no]: # use this if you want to push your own website source code; 'no' is default

AWS CLI profile name [default]: # the AWS account profile name saved in your local AWS CLI configuration; often this is called 'default'

AWS region [us-east-1]: # which region you want to build the supporting resources in, such as EC2 and database resources

Existing EC2 key pair name [replace-with-your-key-pair]: # type an existing EC2 key pair from your account

SSH allowed CIDR [0.0.0.0/0]: # if you know which public IP/CIDR range you want to allow to your instance, type it here; otherwise leave the default and tighten security settings later

EC2 instance type [t3.micro]: # default simple EC2 instance type; depending on goals, this may change to improve performance
```
## 3. Confirming AWS Resources from Terraform
Once set, the script will start Terraform, scaffold resources, and prepare to build them in AWS. Before doing so, the script will ask if these resources are okay. Type `yes` to continue:
![create](../images/wp-lite/create_resources.png)

## 4. Cloud Built, Checking Endpoints

The script will pivot away from the Bash prompt and start running Terraform output, so do not be concerned if it hangs for a bit. Below is a sped-up version after you type `yes` to confirm Terraform:
...
Then all login information for both MariaDB and the WordPress admin page, along with the proper endpoint URLs, is displayed for you to click through:
![outputs_final](../images/wp-lite/outputs-final.png)

⚠️ You do not need to type in a password. `openssl` will generate one for you, and then the script will display it in the terminal.

## 6. Explore WordPress
Once the script is done, you can access the public WordPress landing page and log in using the information displayed by the terminal:
![WP Admin Login](../images/gifs/gif5_wp-admin.gif)
⚠️ You will be taken to the placeholder website saved in the `website/` directory at the project root. You can alter it as a boilerplate if needed.
